/**
 * TYTX Base - Core Tests (TypeScript)
 *
 * @license Apache-2.0
...
describe('TYTX Base TypeScript - Encode', () => {
...
describe('TYTX Base TypeScript - Decode', () => {
    ...
    it('should decode Date', () => {
        const result = fromText<{ d: Date }>('{"d":"2025-01-15::D"}::JS');
    ...
    it('should decode DateTime', () => {
        const result = fromText<{ dt: Date }>('{"dt":"2025-01-15T10:30:00Z::DHZ"}::JS');
    ...
    it('should decode deprecated DH', () => {
        const result = fromText<{ dt: Date }>('{"dt":"2025-01-15T10:30:00::DH"}::JS');
        assert.ok(result.dt instanceof Date, 'Should be Date instance');
    });

    it('should decode Time', () => {
        const result = fromText<{ t: Date }>('{"t":"10:30:00::H"}::JS');
    ...
    it('should decode Boolean', () => {
        const result = fromText<{ flag: boolean }>('{"flag":"1::B"}::JS');
        assert.equal(result.flag, true);
    });

    it('should decode Integer (L)', () => {
        const result = fromText<{ count: number }>('{"count":"42::L"}::JS');
        assert.equal(result.count, 42);
        assert.ok(Number.isInteger(result.count));
    });

    it('should decode Float (R)', () => {
        const result = fromText<{ value: number }>('{"value":"3.14::R"}::JS');
    ...
    it('should not hydrate dict without ::JS marker', () => {
        ...
        const result = fromText<{ price: string }>('{"price":"100.50::N"}');
    ...
    it('should hydrate quoted scalar with type suffix', () => {
        ...
        const result = fromText<Date>('"2025-01-15::D"');
    ...
    it('should hydrate quoted Decimal scalar', () => {
        const result = fromText<DecimalValue | number>('"100.50::N"');
        if (DecimalLib) {
            assert.ok(isDecimalInstance(result), 'Should be Decimal instance');
            assert.equal(Number((result as DecimalValue).toString()), 100.50);
        } else {
    ...
    it('should hydrate quoted DateTime scalar', () => {
        const result = fromText<Date>('"2025-01-15T10:30:00.000Z::DHZ"');
        assert.ok(result instanceof Date, 'Should be Date instance');
    ...
    it('should handle unknown suffix', () => {
        const result = fromText<{ value: string }>('{"value":"test::UNKNOWN"}::JS');
        assert.equal(result.value, 'test::UNKNOWN', 'Should remain as string');
    });

    it('should handle nested structures', () => {
        const result = fromText<{ a: { b: DecimalValue | number } }>('{"a":{"b":"100::N"}}::JS');
        if (DecimalLib) {
    ...
    it('should handle arrays', () => {
        const result = fromText<(DecimalValue | number)[]>('["1.0::N","2.0::N"]::JS');
        assert.ok(Array.isArray(result));
        assert.equal(result.length, 2);
    });
});

describe('TYTX Base TypeScript - JSON Format (with TYTX:// prefix)', () => {
    ...
    it('should decode with TYTX:// prefix', () => {
        const result = fromJson<{ price: DecimalValue | number }>('TYTX://{"price":"100.50::N"}::JS');
        if (DecimalLib) {
    ...
    it('should decode without TYTX:// prefix', () => {
        const result = fromJson<{ price: DecimalValue | number }>('{"price":"100.50::N"}::JS');
        if (DecimalLib) {
            assert.ok(isDecimalInstance(result.price));
        } else {
            assert.equal(result.price, 100.50);
...
describe('TYTX Base TypeScript - Roundtrip', () => {
    ...
    it('should roundtrip Date', () => {
        ...
        const encoded = toTypedText(original);
        const decoded = fromText<{ d: Date }>(encoded);
    ...
    it('should roundtrip complex structure', () => {
        ...
        const encoded = toTypedText(original);
        const decoded = fromText<typeof original>(encoded);
        assert.equal(decoded.invoice.date.getTime(), original.invoice.date.getTime());
    });
});

describe('TYTX Base TypeScript - Registry', () => {
