...
- `js/`: builder + test `node:test` per JSON (text e TYTX://).

## Dataset
- Copre scalari nativi e speciali (Decimal, date, datetime naive/UTC, time con microsecondi, bool, int/float, string, null).
...
```bash
# Python
pytest full-test-suite/python/test_full_suite.py

# JS
node --test full-test-suite/js/test_full_suite.js
```
