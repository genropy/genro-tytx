...
"""
TYTX HTTP Utilities and Middleware.

Content-Type headers, request/response helpers, and ASGI/WSGI middleware
for TYTX payloads.
"""
...
from .encode import _to_json
from .decode import _from_json
    ...
    async def __call__(
        ...
        if self.decode_body:
            ...
            if "json" in content_type or "tytx" in content_type:
                ...
                if body_bytes:
                    try:
                        tytx["body"] = _from_json(body_bytes.decode("utf-8"))
            ...
            if "json" in content_type or "tytx" in content_type:
                try:
                    ...
                    if content_length > 0:
                        ...
                        tytx["body"] = _from_json(body.decode("utf-8"))
                    else:
                        tytx["body"] = None
                except (json.JSONDecodeError, ValueError):
                    tytx["body"] = None
